...
class JSON_Serializable_Vector : public std::vector<std::shared_ptr<ObjectType>>, public JSON_Serializable {
public:
    ...
    size_t indexOf(ObjectType *searchForPtr) {
        int index = 0;
        for (std::shared_ptr<ObjectType> &ptr: *this) {
            if (&(*ptr) == searchForPtr) {
                return index;
            }
            ++index;
